...
Page({
  ...
  data: {
    ...
    onclickordid:''//点击购买邀请码时用的orderid
  },
  toReceipt(e) {
    app.alerts("您确定已收到货物了吗？", {
      successBack: () => {
        util.http({
          url: app.globalData.siteUrl + '/Main/Member/ReceiptOrder?devicetype=3&uid=' + wx.getStorageSync('SessionUserID'),
          method: "POST",
          data: {
  ...
  toPay(e, onec) {
    if (e.currentTarget.dataset.packageid && wx.getStorageSync('fxshopid') == '' && !this.data.showcodes){
      this.setData({
        showcodes: e.currentTarget.dataset.invitationcode,
        onclickordid: e.currentTarget.dataset.orderid
      })
      this.tianxieyaoqing()    
      return   
    }
    ...
    util.http({
      url: app.globalData.siteUrl + '/Main/ShoppingSinaPay/GetPayOrderInfo?devicetype=3',
      data: {
        uid: wx.getStorageSync('SessionUserID'),
        orderid: e.currentTarget.dataset.orderid||e,
        wxapplet: '1',//小程序标识
        devicemac: wx.getStorageSync("SessionUserID"),
        payid: '20180929145421231232'
      },
      header: 1,
      successBack: (ret) => {
        console.log(ret)
  ...
  cancleOrder(e) {
    app.alerts("确认关闭订单?", {
      successBack: () => {
        util.http({
          url: app.globalData.siteUrl + '/Main/Member/CloseOrder?devicetype=3&uid=' + wx.getStorageSync('SessionUserID'),
          method: "POST",
          data: {
            uid: wx.getStorageSync('SessionUserID'),
  ...
  getMsg(type) {
    ...
    util.http({
      url: app.globalData.siteUrl + '/Main/Member/GetOrderListJson?devicetype=3',
      data: {
        uid: wx.getStorageSync('SessionUserID'),
        currpage: this.data.page[this.data.activeTab],
        pagesize: this.data.pagesize,
  ...
  showcode(e) {
    ...
    let a={
      currentTarget:{
        dataset:{
          orderid: that.data.onclickordid
        }
      }
    }
    this.toPay(a,'1')
