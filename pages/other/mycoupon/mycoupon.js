...
Page({
  ...
  refreshMsg() {
	...
	goCanUse(e) {
		if (this.data.slide != 0) return
		var index = e.currentTarget.dataset.index
		var listData = this.data.listMsg[0]
		var data = listData[index]
		if (data.couponType == 6) return
		if (data.isCurrencyCoupon) {
			if (data.couponType == 7) {
				wx.navigateTo({
			...
			if (data.couponType == 7) {
				type = 7
				wx.navigateTo({
					url: '/pages/prolist/prolist?couponid=' + data.couponid + '&type=' + type,
				})
			} else {
				wx.navigateTo({
					url: '/pages/prolist/prolist?couponid=' + data.couponid + '&type=' + type,
