...
Page({
  ...
  oks(){
    util.http({
      url: app.globalData.siteUrl + '/Main/Member/RefuncOrder?devicetype=3&uid=' + wx.getStorageSync('SessionUserID'),
      method: "POST",
      data: {
  ...
  onShow: function () {
    util.http({
      url: app.globalData.siteUrl + '/Main/Member/GetUserCashWithdrawalWay?devicetype=3',
      data: {
        uid: wx.getStorageSync('SessionUserID'),
        orderid: this.data.options.orderid
      },
