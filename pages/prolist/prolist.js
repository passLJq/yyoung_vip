...
Page({
  ...
  onLoad: function (options) {
    ...
    if (wx.getStorageSync('ruid')!=''){
      ...
      that.setData({
		...
		// 有优惠券id就读关联商品
		if (options.couponid) {
			this.getCouPro(options.couponid, options.type)
		} else {
			that.bindata()
		}
    this.setData({
  ...
  usermeng:function (ret) {
	...
	getCouPro(coupon, type) {
		utils.http({
			url: app.globalData.siteUrl + '/Marketing/Coupon/getCouponInProduct',
			data: {
  ...
  onShareAppMessage: function () {
    ...
    if (wx.getStorageSync('ruid') != '') {
      ...
      ruid = wx.getStorageSync('SessionUserID')
		...
		var pid = that.data.lisdata[that.data.sharpindex].pid || that.data.lisdata[that.data.sharpindex].proId
    return {
      title: that.data.lisdata[that.data.sharpindex].name,
      path: 'pages/productdetail/productdetail?pid=' + pid + '&ruid=' + ruid,
