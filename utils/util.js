...
const CheckLoginStatus = require("./CheckLoginStatus.js")
...
// 判断ruid是否开店
function isOpenRuid(ruid, callback) {
	return new Promise((res, rej) => {
		http({
			url: app_utils.globalData.siteUrl + '/Main/Member/CheckShop',
			data: {
				uid: ruid
			},
			header: 1,
			successBack: ret => {
				ret = ret.data
				if (ret.success && ret.status == 0) {
					// 没开店情况清空ruid缓存 并传回数据
					wx.setStorageSync('ruid', '')
					res(ruid)
				} else if (ret.success && ret.status == 1 && ret.Data.fxshopid) {
					// 开店了则存入ruid缓存
...
var AnalysisAdsUrl = {
  clickAds: function (type, title, ref) {
    ...
    switch (type) {
      ...
      case '4': //栏目
        wx.navigateTo({
          url: '/pages/other/aboutmain/aboutmain?columnid=' + ref
        }) 
        break;
      case '9': //一些特殊链接
...
function xlang_ordernow(orderid, ogid, issuccess) {
  http({
    url: app_utils.globalData.siteUrl +'/Main/ShoppingSinaPay/OrderPayStatus?devicetype=3',
    data: {
      orderid: orderid,
      uid: wx.getStorageSync("SessionUserID"),
      ogid: ogid,
